...
function Pantun() {
	const pantun = fs.readFileSync('./lib2/database/pantun.json')
	const data = JSON.parse(pantun)
	const acak = data[Math.floor(Math.random() * (data.length))]
	return acak
}
function trutdare(querry) {
	const trdr = fs.readFileSync('./lib2/database/truthdare.json') // tempat simpen file lu
...
function FaktaUnik() {
	const fakta = fs.readFileSync('./lib2/database/faktaunik.json')
...
async function cuaca(wilayah) {
	...
	function Cuaca(daerah) {
		const listcuaca = fs.readFileSync('./lib2/database/cuaca.json')
