...
router.get('/ttstalk', async(req, res, next) => {
	...
	if (!username) return res.json({ status: false, code: 404, error: 'masukan param username'})
	const tt = require('tiktok-scraper')
	const cih = await tt.getUserProfileInfo(username)
	res.json(cih)
	})
...
router.get('/igstalk', async(req, res, next) => {
	...
	const upi = await ig.getUserMeta(username)
	res.json(upi)
	})
...
router.get('/stalk/igstalk', async (req, res, next) => {        
